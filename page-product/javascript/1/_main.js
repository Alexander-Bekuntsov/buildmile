...
new Swiper('.catalog-swiper', {
    ...
    spaceBetween: 5,
    navigation: {
        nextEl: '.rcatalog__nav-item.item--next',
        prevEl: '.rcatalog__nav-item.item--prev',
    },
    breakpoints: {
        1250: {
            slidesPerView: 2,
...
$(document).on("click", ".js-sort-social .filter-sort__nav li", function () {
    ...
    Parent.parent().parent().find(".project__chr-pane-price").text($(this).data('value') + ' ₽');
});
...
$(document).on("click", ".project__slider-nav img:not(.icon-play)", function () {
